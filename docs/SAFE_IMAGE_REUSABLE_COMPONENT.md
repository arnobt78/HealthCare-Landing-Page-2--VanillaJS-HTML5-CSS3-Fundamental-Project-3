# SafeImage — `next/image` with native `<public/images>` fallback

Use this pattern in **any Next.js App Router project** where you show **remote images** (Unsplash, CDNs, OpenWeather icons, etc.). If **Next.js / Vercel Image Optimization** hits limits or errors (e.g. **HTTP 402** on the free tier, quota exceeded, or upstream failures), the default `<Image>` request can break. **`SafeImage`** tries **`next/image` first**; on **`onError`**, it re-renders with a plain **`<public/images src="…">`** pointing at the **same URL**, so the image usually **still displays** (no optimizer in the middle).
...
4. **ESLint** — `@next/next/no-public/images-element` may warn on the fallback `<public/images>` lines; keep the **inline eslint-disable** comments (they document _why_ `<public/images>` is intentional).
...
export function SafeImage({
  ...
  if (useNative && resolvedSrc) {
    if (fill) {
      return (
        // eslint-disable-next-line @next/next/no-public/images-element -- fallback when /_next/image fails (e.g. 402)
        <public/images
          alt={alt}
          src={resolvedSrc}
...
| Load fails              | `onError` runs → state flips → **`<public/images>`** with **original string `src`** (browser loads URL directly; **not** via `/_next/image`). |
| Optional `onError` prop | Still called before switching to native public/images.                                                                                        |

---

## For AI / collaborators (paste this instruction)

> In this Next.js app, **do not use raw `next/image` for remote URLs** in user-facing UI unless there is a reason. Prefer **`SafeImage`** from `@/…/safe-image.tsx`: it wraps **`next/image`** and falls back to an **unoptimized HTML `<public/images>`** with the same `src` when the optimized image request fails (e.g. Vercel Image Optimization quota / 402). Keep **`remotePatterns`** in `next.config` in sync with image domains. For **`fill`**, parent must be **`relative`** with explicit size; pass **`sizes`** when using fill for layout hints.
