...
- **Assets folder:** If you use a different folder than `public` (e.g. `assets`, `public/images`), change the last line to `copyDir(path.join(root, "assets"), path.join(dist, "assets"))` (and ensure your HTML references the same paths).
